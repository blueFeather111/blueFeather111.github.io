...
我猜测offset的意义是这样的，把原图分成若干个8x8的网格，每个网格中取左上角像素作为代表点，它的offset为(0,0), 其他点坐标用左上角的点+offset计算。那么这些代表点就形成了原图resize到下采样8倍的尺寸。代表点为粗匹配点。offset为精匹配。

![图片描述](/assets/image/9.png)
![图片描述](/assets/image/10.png)           

ground truth中为图片1和图片2的互相匹配的特征点坐标，把坐标转化为粗匹配和offset后，用粗匹配坐标从F1，F2中采样到对应的特征点的64维的描述子fa, fb。
